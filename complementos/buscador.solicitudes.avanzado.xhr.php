<?php 
/*
 * BUSCADOR AVANZADO DE SUSCRIPTORES V.0.1
...
if ($permiso) {
 ...
 $formulario->campos['criterio'] = $solicitudes->combo_campos("criterio", @$_REQUEST['campo']);
 ...
 $formulario->campos['basica'] = $formulario->button("basica", "button", "Básica ");
 $formulario->celdas['criterio'] = $formulario->celda("Criterio De Busqueda:", $formulario->campos['criterio']);
 $formulario->celdas['busqueda'] = $formulario->celda("Valor Buscado", $formulario->campos['busqueda']);
 $formulario->fila($formulario->celdas['criterio']);
 $formulario->fila($formulario->celdas['busqueda']);
 $formulario->botones($formulario->campos['buscar']);
 $formulario->botones($formulario->campos['basica']);
 echo($formulario->generar());
 echo($formulario->cierre());
 echo($formulario->JavaScript("MUI.Suscriptores_Solicitudes_Buscar_Avanzado('" . @$_REQUEST['criterio'] . "','" . @$_REQUEST['busqueda'] . "');"));
$formulario->eClick("basica", "MUI.Suscriptores_Solicitudes_Buscador();");
} else {
 /**
